defmodule SimpleApiWeb.UserControllerTest do
  ...
  @current_users_attrs %{email: "current user email", is_active: true, password: "some current user password"}

  def fixture(:current_user) do
    { :ok, current_user } = Auth.create_user(@current_users_attrs)
    current_user
  end
  ...
  describe "sign_in user" do
    test "renders user when user credentials are good", %{conn: conn, current_user: current_user} do
      conn = post conn, user_path(conn, :sign_in, %{ email: current_user.email, password: @current_users_attrs.password })
      assert json_response(conn, 200)["data"] == %{
        "user" => %{
          "id" => current_user.id,
          "email" => current_user.email }}
